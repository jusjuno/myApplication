...
import java.util.StringTokenizer; //java.util.StringTokenizer;
...
public class TalkServer implements ActionListener {
	...
	public void waitClient() {
		...
		while (true) {
			...
			try {
				String msg = dis.readUTF().trim(); // trim은 좌우 공백을 없애줌
				String msgPlus = "";
				...
				while (st.hasMoreTokens()) { // token이 있는가?
					msgPlus += st.nextToken();
				} // 이 while 문이 없으면 / 다음의 내용이 짤린다.
				view.append(msgPlus + "\n");
			} catch (Exception e) {
